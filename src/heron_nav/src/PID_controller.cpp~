...
#include<robotx_control/pidConfig.h>
...
void callback(robotx_control::pidConfig &config)
{
...
PIDController::PIDController():linear_prev_error(0),angular_prev_error(0),linear_integral(0),angular_integral(0),prev_time(ros::Time::now()),now(ros::Time::now())
{
	...
	odom_sub = n.subscribe<nav_msgs::Odometry>("odometry/filtered/global",10,&PIDController::OdomCallback,this);
	drive_pub = n.advertise<kingfisher_msgs::Drive>("cmd_drive",10);
}
...
int main(int argc, char** argv)
{
	...
	dynamic_reconfigure::Server<robotx_control::pidConfig> server;
	dynamic_reconfigure::Server<robotx_control::pidConfig>::CallbackType f;
