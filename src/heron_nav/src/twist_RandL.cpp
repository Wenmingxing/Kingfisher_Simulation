...
#include <robotx_control/controlConfig.h>
...
void callback(robotx_control::controlConfig& config, uint32_t level){
...
int main(int argc,char** argv)
{
	...
	ros::NodeHandle n;
	dynamic_reconfigure::Server<robotx_control::controlConfig> server;
	dynamic_reconfigure::Server<robotx_control::controlConfig>::CallbackType f;
	f = boost::bind(&callback,_1,_2);
	server.setCallback(f);

	ros::Subscriber cmd_vel_sub = n.subscribe("cmd_vel",10,cmd_velCallback);
	ros::Publisher RandL_pub = n.advertise<kingfisher_msgs::Drive>("cmd_drive",10);
